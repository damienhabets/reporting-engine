...
objects
    Browse records the report is being run on
get_odoo_param
    Function that allows reading Odoo parameters (such as ``web.base.url``). Use with ``function=get_odoo_param('web.base.url')`` with ``py3o://function=get_odoo_param('web.base.url')`` as a link around it.
user
